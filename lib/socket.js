 ...
 *
 * @param {Array} rooms
 * @param {Function} Merge method
 * @param {Function} optional, callback
 * @return {Socket} self
 * 
 */
 
Socket.prototype.joinAll = function(rooms, merge_fn, fn){
  // debug('joining room %s', room);
  var self = this;
  var diff = function(a) {
  ...
  var socket_rooms = this.rooms;
  diff.apply(rooms, [socket_rooms]);
  if (rooms.length <= 0) return this;
  // if (~this.rooms.indexOf(room)) return this;
  this.adapter.addAll(this.id, rooms, merge_fn, function(err){
    if (err) return fn && fn(err);
    self.rooms = self.rooms.concat(rooms);
    // console.log('joined rooms in SocketIO: ', self.rooms);
    fn && fn(null);
  });
  return this;
};
